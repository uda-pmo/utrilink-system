...
app.post('/api/orders', auth, async (req, res) => { const record = orderRecord(req.body); if (!record.product_name || !record.contract_no || !record.factory_name) return res.status(400).json({ error: '产品名称、合同号和工厂名称为必填项。' }); const { data, error } = await supabase.from('nl_orders').insert(record).select().single(); if (error) return res.status(error.code === '23505' ? 409 : 500).json({ error: error.code === '23505' ? '合同号已存在。' : error.message }); res.status(201).json(data); });
app.put('/api/orders/:id', auth, async (req, res) => { const record = orderRecord(req.body); if (!record.product_name || !record.contract_no || !record.factory_name) return res.status(400).json({ error: '产品名称、合同号和工厂名称为必填项。' }); record.updated_at = new Date().toISOString(); const { data, error } = await supabase.from('nl_orders').update(record).eq('id', req.params.id).select().maybeSingle(); if (error) return res.status(error.code === '23505' ? 409 : 500).json({ error: error.code === '23505' ? '合同号已存在。' : error.message }); if (!data) return res.sendStatus(404); res.json(data); });
