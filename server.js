...
app.get('/api/orders', auth, async (req, res) => { const sortColumns = { product: 'product_name', factory: 'factory_name', sku: 'sku', node: 'node', progress: 'progress', due: 'due_date', status: 'status', updated: 'updated_at' }; const sortColumn = sortColumns[String(req.query.sort || '')] || 'updated_at'; const ascending = String(req.query.direction || 'desc') !== 'desc'; let query = supabase.from('nl_orders').select('*').order(sortColumn, { ascending, nullsFirst: false }); if (isFactory(req.user)) query = query.eq('factory_name', req.user.factory_name); if (req.query.factory) query = query.eq('factory_name', String(req.query.factory)); if (req.query.status) query = query.eq('status', String(req.query.status)); const search = String(req.query.search || '').trim().replace(/[,%()]/g, ' '); if (search) query = query.or(`product_name.ilike.%${search}%,factory_name.ilike.%${search}%,sku.ilike.%${search}%,contract_no.ilike.%${search}%`); const { data, error } = await query; if (error) return fail(res, error); res.json(data); });
