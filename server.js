...
app.put('/api/orders/:id/milestones/:milestoneId', auth, async (req, res) => {
  ...
  await notify({ orderId: order.id, kind: '里程碑更新', title: `${req.user.name} 在“${milestone.node_name}”更新了里程碑`, detail: `${milestone.status || '待开始'}${milestone.delay_reason ? `：${milestone.delay_reason}` : ''}`, targetRole: isFactory(req.user) ? 'brand' : 'factory', actor: req.user });
  res.json(milestone);
});
